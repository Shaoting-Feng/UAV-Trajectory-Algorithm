...
class UavTrajectory(gym.Env):
    ...
    def __init__(self, render_mode="console"):
        ...
        # modified here
        obv_low = np.zeros((obj_num+1, 1))
        obv_low[0] = -np.inf
        obv_high = np.full((obj_num+1, 1), np.inf)
        self.observation_space = spaces.Box(
            # edited on 07.29 for Version 1 - 2 
            low=obv_low, high=obv_high, dtype=np.float32
        )
    ...
    def getObs(self):
        # modified here
        obs = np.zeros((obj_num+1, 1))
        obs[0] = self.x_UAV
        i = 1
        for item in self.x_obj_list:
            obs[i] = item
            i = i + 1
        return obs
